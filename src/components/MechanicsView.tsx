...
export default function MechanicsView({
  ...
  const [deleteConfirmMechanic, setDeleteConfirmMechanic] = useState<Mechanic | null>(null);
  ...
  const handleConfirmDelete = async () => {
    if (!deleteConfirmMechanic) return;
    setIsDeleting(true);
    try {
      await onDeleteMechanic(deleteConfirmMechanic.id);
      setDeleteConfirmMechanic(null);
    } catch (err: any) {
      console.error(err);
      alert(err?.message || 'خطا در حذف تعمیرکار');
    } finally {
      setIsDeleting(false);
    }
  };
            ...
            <div className="flex items-center justify-between gap-2 pt-3 border-t border-slate-200 dark:border-[#2d2d30]">
              {editingMechanicId ? (
                <button
                  type="button"
                  onClick={() => {
                    const currentM = mechanics.find(m => m.id === editingMechanicId);
                    if (currentM) {
                      setIsFormOpen(false);
                      setDeleteConfirmMechanic(currentM);
                    }
                  }}
  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-300">
      ...
      {/* مدال تایید حذف تعمیرکار */}
      <DeleteConfirmModal
        isOpen={deleteConfirmMechanic !== null}
        title="حذف تعمیرکار"
        subtitle={deleteConfirmMechanic ? deleteConfirmMechanic.name : ''}
        message={deleteConfirmMechanic ? `آیا از حذف تعمیرکار «${deleteConfirmMechanic.name}» اطمینان دارید؟` : ''}
        isDeleting={isDeleting}
        onConfirm={handleConfirmDelete}
        onClose={() => {
          if (!isDeleting) setDeleteConfirmMechanic(null);
        }}
      />
    </div>
  );
